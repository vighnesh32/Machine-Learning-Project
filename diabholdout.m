...
histogram(out_counts,'FaceColor','#EDB120')
xlabel('Outcome')
ylabel('Counts')
...
cv = cvpartition(size(df_dia3,1),'HoldOut',0.2);
idx = cv.test;
...
cv1 = cvpartition(size(df_dia4,1),'HoldOut',0.2);
idx = cv1.test;
train_data1 = df_dia4(~idx,:);
test_data1  = df_dia4(idx,:);
...
title('ROC for Decision Tree and Naïve Bayes')
hold off
legend('Decision Tree', 'Naïve Bayes ',Location='best')
...
sgtitle('Holdout percentage splitting')